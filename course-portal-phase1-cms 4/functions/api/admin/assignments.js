...
export async function onRequestPost({ request, env }) {
  ...
  const row = await env.DB.prepare("SELECT COALESCE(MAX(sort_order), 0) as max FROM assignments WHERE unit_id = ?")
    .bind(unit_id)
    .first();

  const inserted = await env.DB.prepare(
    "INSERT INTO assignments (unit_id, title, sort_order) VALUES (?, ?, ?) RETURNING id"
  )
